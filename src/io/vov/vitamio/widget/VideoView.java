...
public class VideoView extends SurfaceView implements
	...
	private static final int STATE_PREPARING = 1;// 正在准备
	private static final int STATE_PREPARED = 2;// 准备
	private static final int STATE_PLAYING = 3; // 播放
	private static final int STATE_PAUSED = 4; // 暂停
	private static final int STATE_PLAYBACK_COMPLETED = 5;// 播放完成返回
	private static final int STATE_SUSPEND = 6; // 暂停
	private static final int STATE_RESUME = 7; // 继续
	private static final int STATE_SUSPEND_UNSUPPORTED = 8; // 不支持暂停

	private int mCurrentState = STATE_IDLE;
	private int mTargetState = STATE_IDLE; // 闲置状态

	private float mAspectRatio = 0;
	private int mVideoLayout = VIDEO_LAYOUT_SCALE;
	public static final int VIDEO_LAYOUT_ORIGIN = 0; // 原始模式
	public static final int VIDEO_LAYOUT_SCALE = 1; // 旋转模式
	public static final int VIDEO_LAYOUT_STRETCH = 2; // 拉伸模式
	public static final int VIDEO_LAYOUT_ZOOM = 3; // 缩放模式

	private SurfaceHolder mSurfaceHolder = null;
	private MediaPlayer mMediaPlayer = null;
	private int mVideoWidth; // 宽度
	private int mVideoHeight; // 高度
	private float mVideoAspectRatio; // 纵横比
	private int mSurfaceWidth; // 宽度
	private int mSurfaceHeight; // 高度
	private MediaController mMediaController; // 控制台对象
	private OnCompletionListener mOnCompletionListener; // 播放结束监听
	private OnPreparedListener mOnPreparedListener; // 准备监听
	private OnErrorListener mOnErrorListener; // 错位监听
	private OnSeekCompleteListener mOnSeekCompleteListener; // seek完成监听
	private OnSubtitleUpdateListener mOnSubtitleUpdateListener; // 更新完成监听
	...
	private void initVideoView(Context ctx) {
		...
		if (ctx instanceof Activity) // 获取音量控制
			((Activity) ctx).setVolumeControlStream(AudioManager.STREAM_MUSIC);
	}

	public boolean isValid() { // 判断是否有效
	...
	/*
	 * 获取当前播放的duration(non-Javadoc)
	 * 
	 * @see
	 * io.vov.vitamio.widget.MediaController.MediaPlayerControl#getDuration()
	 */
	@Override
	...
	/*
	 * 获取当前的播放位置(non-Javadoc)
	 * 
	 * @see
	 * io.vov.vitamio.widget.MediaController.MediaPlayerControl#getCurrentPosition
	 * ()
	 */
	@Override
	...
	public void seekTo(long msec) {
		if (isInPlaybackState()) {
			Log.e("seekToprogress=====" + msec, msec);
