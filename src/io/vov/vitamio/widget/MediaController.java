...
public class MediaController extends FrameLayout {
	...
	public MediaController(Activity activity, VideoView videoView) {
		...
		if (!mFromXml && initController(activity))
			initFloatingWindow();
	}
	...
	public PlayControl getmPlayControl() {
		return mPlayControl;
	}

	public void setmPlayControl(PlayControl mPlayControl) {
		this.mPlayControl = mPlayControl;
	}

	/**
	 * 设置控制视图 Set the view that acts as the anchor for the control view. This
	 * can for example be a VideoView, or your Activity's main view.
	 * 
	 * @param view
	...
	protected View makeControllerView() {
		...
		if (mCurrentOrientation == Configuration.ORIENTATION_PORTRAIT) {
			layout = R.layout.vitamio_mediacontroller_top;
		} else if (mCurrentOrientation == Configuration.ORIENTATION_LANDSCAPE) {
			layout = R.layout.vitamio_mediacontroller;
		}
	...
	private void initControllerView(View v) {
		...
		mIv_pause = (ImageView) v.findViewById(R.id.ib_pause);
		mIv_pause.setOnClickListener(mPauseListener);
		...
		mIb_back = (ImageButton) v.findViewById(R.id.ib_back);
		mIb_back.setOnClickListener(mBackListener);
		...
		mTv_PlayName = (TextView) v.findViewById(R.id.mediacontroller_name);
		mTv_PlayName.setText(mTitle);
		mFillButton = (ImageButton) v.findViewById(R.id.mediacontroller_fill);
		if (mFillButton != null) {
	...
	private onPauseListener mOnPauseListenerl;

	public void setOnPauseListener(onPauseListener pauseListener) {
		mOnPauseListenerl = pauseListener;
	...
	private View.OnClickListener mCollectListener = new View.OnClickListener() {
		@Override
		public void onClick(View v) {
			mPlayControl.collect();
		}
	};

	private View.OnClickListener mLastListener = new OnClickListener() {
	...
	private View.OnClickListener mFillListener = new OnClickListener() {
	...
	private void updatePausePlay() {
		...
		if (mPlayer.isPlaying()) {
			mPauseButton.setImageResource(R.drawable.vitamio_play);
			mIv_pause.setImageResource(R.drawable.vitamio_ib_play);
		} else {
			mPauseButton.setImageResource(R.drawable.vitamio_pause);
			mIv_pause.setImageResource(R.drawable.vitamio_video_play_btn);
		}
	}
	...
	private void doPauseResume() {
		if (mPlayer.isPlaying()) {
			mPlayer.pause();
			mOnPauseListenerl.onPause();
			mIv_pause.setImageResource(R.drawable.vitamio_video_play_btn);
		} else {
			mPlayer.start();
			mOnPauseListenerl.onPlay();
			mIv_pause.setImageResource(R.drawable.vitamio_ib_play);
		}
		updatePausePlay();
	}
